...
public class GlowPadView extends View {
	...
	private int mEnabledTargetCount = 0;
		...
		for (int k = 0; k < historySize + 1; k++) {
			...
			if (mDragging) {
				...
				for (int i = 0; i < ntargets; i++) {
					...
					if (target.isEnabled()) {
						boolean angleMatches = (mEnabledTargetCount == 1 && mMagneticTargets) ||
	...
	public void setEnableTarget(int resourceId, boolean enabled) {
		for (int i = 0; i < mTargetDrawables.size(); i++) {
			...
			boolean prevState = target.isEnabled();
			if (target.getResourceId() == resourceId && prevState != enabled) {
				target.setEnabled(enabled);
				mEnabledTargetCount += enabled ? 1 : -1;
	...
	public void setMagneticTargets (boolean useMagneticTargets){
		mMagneticTargets = useMagneticTargets;
	}
